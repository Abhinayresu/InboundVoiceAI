...
app = FastAPI(title="Abhinay Resu's AI Dashboard")
...
def read_config():
    ...
    return {
        "first_line": get_val("first_line", "FIRST_LINE", "Namaste! This is Abhinay's AI agent — we help businesses automate with AI. Hmm, may I ask what kind of business you run?"),
...
<head>
  ...
  <title>AI Voice Demo — Abhinay Resu</title>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap" rel="stylesheet">
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{font-family:'Inter',sans-serif;background:#050505;color:#e2e8f0;display:flex;align-items:center;justify-content:center;min-height:100vh;flex-direction:column;gap:24px;padding:24px}
    .card{background:#111111;border:1px solid #27272a;border-radius:20px;padding:40px;max-width:440px;width:100%;text-align:center;box-shadow:0 24px 60px rgba(0,0,0,0.4)}
    h1{font-size:22px;font-weight:700;margin-bottom:6px}
    .sub{color:#8892a4;font-size:13px;margin-bottom:28px}
    .avatar{width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,#ff6b00,#ff9d00);display:flex;align-items:center;justify-content:center;font-size:36px;margin:0 auto 24px}
    ...
    .btn-start{background:#ff6b00;color:#fff}
    .btn-start:hover{background:#e65c00;box-shadow:0 0 24px rgba(255,107,0,0.4)}
    ...
    .vol-bar span{width:4px;background:#ff6b00;border-radius:2px;transition:height 0.1s}
  
    @property --hue { syntax: "<number>"; inherits: true; initial-value: 20; }
    ...
    .glow-btn.w-auto { width: auto; min-width: 160px; }
    ...
    .glow-content {
      ...
      padding: 10px 18px; font-size: 13px; font-weight: 600; color: var(--text-color);
    ...
    .glow-content:before {
      ...
      border-radius: calc(var(--card-radius) * 0.9); box-shadow: 0 0 20px black;
    ...
    .glow {
      --glow-translate-y: 0; display: block; position: absolute;
      width: 40px; height: 40px; animation: rotate var(--animation-speed) linear infinite;
    ...
    .glow:after {
      ...
      background: hsl(calc(var(--hue) * var(--hue-speed) * 1deg) 100% 60%); position: relative;
      border-radius: calc(var(--glow-radius) * 10vw); animation: hue-animation var(--animation-speed) linear infinite;
    ...
    .glow-btn:hover .glow-content {
      mix-blend-mode: darken; --text-color: white;
      box-shadow: 0 0 calc(var(--white-shadow) * 1vw) calc(var(--white-shadow) * 0.15vw) rgb(255 255 255 / 20%);
      animation: shadow-pulse calc(var(--animation-speed) * 2) linear infinite;
    }

    .glow-btn:hover .glow-content:before {
      --bg-size: 15; animation-play-state: paused; transition: --bg-size var(--interaction-speed) ease;
    }

    .glow-btn:hover .glow {
      --glow-blur: 1.5; --glow-opacity: 0.6; --glow-scale: 2.5; --glow-radius: 0; --rotate: 900;
      --glow-rotate-unit: 0; --scale-factor: 1.25; animation-play-state: paused;
    }

    .glow-btn:hover .glow:after {
      --glow-translate-y: 0; animation-play-state: paused;
...
<body>
  <div class="card">
    ...
    <h1>Talk to Abhinay's AI Agent</h1>
    <div class="sub">AI-powered multilingual consultant · Abhinay Resu</div>
...
def health_check():
    ...
    return {
        ...
        "service": "abhinay-ai-voice-agent",
    }
...
<head>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .btn-primary:hover {{ background: #e65c00; box-shadow: 0 0 16px var(--accent-glow); }}
    ...
    .cal-cell:hover {{ border-color: var(--accent); background: var(--accent-glow); transform: scale(1.03); box-shadow: 0 4px 20px rgba(255,107,0,0.15); }}
    ...
    .stat-card:hover {{ transform: translateY(-3px); box-shadow: 0 8px 30px rgba(255,107,0,0.12); }}
...
<nav id="sidebar">
  <div class="sidebar-brand">
    ...
    <div>
      ...
      <div class="brand-sub">Abhinay Resu</div>
    </div>
  </div>
...
<div id="main">
  ...
  <div id="page-agent" class="page">
    ...
    <div class="section-card">
      ...
      <div class="form-group">
        ...
        <input type="text" id="first_line" value="{config.get('first_line', '')}" placeholder="Namaste! This is Abhinay's AI agent...">
    ...
    <div class="save-bar">
      ...
      <button class="btn glow-btn w-auto"  onclick="saveConfig('agent')">
        <span class="glow"></span>
        <div class="glow-content">💾 Save Agent Settings</div>
      </button>
    </div>
  </div>
  ...
  <div id="page-models" class="page">
    ...
    <div class="save-bar">
      ...
      <button class="btn glow-btn w-auto"  onclick="saveConfig('models')">
        <span class="glow"></span>
        <div class="glow-content">💾 Save Model Settings</div>
      </button>
    </div>
  </div>
  ...
  <div id="page-credentials" class="page">
    <div class="page-header">
      ...
      <div class="page-sub">Configure your external services and keys</div>
    </div>

    <div class="section-card">
      <div class="section-title">LiveKit (WebRTC)</div>
      <div class="form-row">
        <div class="form-group">
          <label>LiveKit URL</label>
          <input type="text" id="livekit_url" value="{config.get('livekit_url', '')}">
        </div>
        <div class="form-group">
          <label>SIP Trunk ID (Optional)</label>
          <input type="text" id="sip_trunk_id" value="{config.get('sip_trunk_id', '')}">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>API Key</label>
          <input type="password" id="livekit_api_key" value="{config.get('livekit_api_key', '')}">
        </div>
        <div class="form-group">
          <label>API Secret</label>
          <input type="password" id="livekit_api_secret" value="{config.get('livekit_api_secret', '')}">
        </div>
      </div>
    </div>

    <div class="section-card">
      <div class="section-title">AI & TTS Models</div>
      <div class="form-row">
        <div class="form-group">
          <label>OpenAI API Key (LLM)</label>
          <input type="password" id="openai_api_key" value="{config.get('openai_api_key', '')}">
        </div>
        <div class="form-group">
          <label>Sarvam API Key (TTS)</label>
          <input type="password" id="sarvam_api_key" value="{config.get('sarvam_api_key', '')}">
        </div>
      </div>
    </div>

    <div class="section-card">
      <div class="section-title">Integrations (Optional)</div>
      <div class="form-row">
        <div class="form-group">
          <label>Cal.com API Key</label>
          <input type="password" id="cal_api_key" value="{config.get('cal_api_key', '')}">
        </div>
        <div class="form-group">
          <label>Cal.com Event Type ID</label>
          <input type="text" id="cal_event_type_id" value="{config.get('cal_event_type_id', '')}">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Telegram Bot Token</label>
          <input type="password" id="telegram_bot_token" value="{config.get('telegram_bot_token', '')}">
        </div>
        <div class="form-group">
          <label>Telegram Chat ID</label>
          <input type="text" id="telegram_chat_id" value="{config.get('telegram_chat_id', '')}">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Supabase URL (CRM Logs)</label>
          <input type="url" id="supabase_url" value="{config.get('supabase_url', '')}">
        </div>
        <div class="form-group">
          <label>Supabase Key</label>
          <input type="password" id="supabase_key" value="{config.get('supabase_key', '')}">
        </div>
      </div>
    </div>
    ...
    <div class="save-bar">
      ...
      <button class="glow-btn btn-primary w-auto" onclick="saveConfig('credentials')">
        <span class="glow"></span>
        <div class="glow-content">💾 Save Credentials</div>
      </button>
    </div>
  </div>
  ...
  <div id="page-outbound" class="page">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
      <div class="section-card">
        ...
        <button class="btn glow-btn"  onclick="makeSingleCall()" style="width:100%;">
        <span class="glow"></span>
        <div class="glow-content">📞 Call Now</div>
      </button>
      ...
      <div class="section-card">
        ...
        <button class="btn glow-btn"  onclick="makeBulkCall()" style="width:100%;">
        <span class="glow"></span>
        <div class="glow-content">🚀 Call All Numbers</div>
      </button>
  ...
  <div id="page-demo" class="page">
    ...
    <div class="section-card" style="max-width:640px;">
      ...
      <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
        <button class="btn glow-btn w-auto"  onclick="generateDemo()">
        <span class="glow"></span>
        <div class="glow-content">✨ Generate Demo Link</div>
      </button>
    ...
    <div class="section-card" style="max-width:640px;margin-top:0;">
      ...
      <iframe id="demo-iframe" src="" style="width:100%;height:520px;border:none;border-radius:12px;background:#050505;display:none;"></iframe>
    ...
    <div class="save-bar">
      <span class="save-status" id="save-status-credentials">✅ Saved!</span>
      <button class="btn glow-btn w-auto"  onclick="saveConfig('credentials')">
        <span class="glow"></span>
        <div class="glow-content">💾 Save Credentials</div>
      </button>
    </div>
  ...
  try {{
    ...
    tbody.innerHTML = contacts.map(c => `
      <tr style="border-bottom:1px solid var(--border);transition:background 0.12s;" onmouseover="this.style.background='rgba(255,255,255,0.025)'" onmouseout="this.style.background=''">
        ...
        <td style="padding:14px 16px;text-align:center;"><span style="background:rgba(255,107,0,0.12);color:var(--accent);padding:3px 10px;border-radius:20px;font-size:12px;font-weight:700;">${{c.total_calls}}</span></td>
...
// ── Language Presets ─────────────────────────────────────────────────────────
const LANG_PRESETS = {{
  hinglish:    {{ flag:'🇮🇳', label:'Hinglish',                sub:'Hindi + English mix',        color:'#ff6b00' }},
  hindi:       {{ flag:'🇮🇳', label:'Hindi',                   sub:'Pure Hindi',                  color:'#ff9d00' }},
...
function renderLangGrid() {{
  ...
  grid.innerHTML = Object.entries(LANG_PRESETS).map(([id, p]) => `
    <div onclick="selectLangPreset('${{id}}')" style="
      background:${{id===currentLangPreset ? 'rgba(255,107,0,0.15)' : 'var(--bg)'}};
      ...
      ${{id===currentLangPreset ? 'box-shadow:0 0 16px rgba(255,107,0,0.2)' : ''}}

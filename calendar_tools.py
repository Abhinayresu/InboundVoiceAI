...
def _get_slots_calcom(date_str: str) -> list:
    ...
    try:
        ...
        slots = []
        for s in raw_slots:
            dt = datetime.fromisoformat(s["time"].replace("Z", "+00:00")).astimezone(ist)
...
async def _create_booking_calcom(
    ...
    valid_email = caller_email.strip() if caller_email.strip() and '@' in caller_email else f"{clean_phone}@voiceagent.com"
...
async def _create_booking_gcal(
    ...
    try:
        ...
        event = {
            ...
            "description": f"Phone: {caller_phone}\nNotes: {notes}\nBooked via Abhinay Resu's AI Voice Agent",
...
def cancel_booking(booking_id: str, reason: str = "Cancelled by caller") -> dict:
    """Cancel a Cal.com booking by UID using API v2."""
    creds = get_cal_creds()
    try:
        resp = requests.post(

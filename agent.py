...
class AgentTools(llm.ToolContext):
    ...
    async def transfer_call(self) -> str:
        ...
        if destination and self.sip_domain and "your_sip_domain" not in self.sip_domain and "@" not in destination:
            clean_dest  = destination.replace("tel:", "").replace("sip:", "")
            destination = f"sip:{clean_dest}@{self.sip_domain}"
        elif destination and destination.startswith("+"):
            destination = f"tel:{destination}"
        elif destination and not destination.startswith("sip:") and not destination.startswith("tel:"):
            destination = f"tel:{destination}"
        try:
    ...
    async def save_booking_intent(
        ...
        notes:       Annotated[str,  "Any notes or special requests"] = "",
    ) -> str:
...
class OutboundAssistant(Agent):
    ...
    async def on_enter(self):
        greeting = self._live_config.get(
            "first_line",
            self._first_line or (
                "Namaste! This is Abhinay from Abhinay's AI Agency — we provide AI solutions for businesses. "
    ...
    async def tts_node(self, text, model_settings):
        """Strip parenthetical English annotations before Sarvam TTS.

        text is AsyncIterable[str] in livekit-agents v1.4. Collect all chunks,
        clean the full string, then re-stream as a fresh async generator.
        """
        import re as _re
        ...
        try:
            ...
            logger.error(f"[TTS-NODE] Error collecting chunks: {e}")
        full_text = "".join(chunks)

        # Strip English parenthetical/bracketed annotations
        # e.g. "నా పేరు అభినయ్. (My name is Abhinay.)" → "నా పేరు అభినయ్."
        cleaned = _re.sub(r'\s*\([^)]*[a-zA-Z]{3,}[^)]*\)', '', full_text)
        cleaned = _re.sub(r'\s*\[[^\]]*[a-zA-Z]{3,}[^\]]*\]', '', cleaned)
        cleaned = cleaned.strip() or full_text

        logger.debug(f"[TTS-NODE] in={repr(full_text[:80])} out={repr(cleaned[:80])}")

        async def _cleaned_gen():
            yield cleaned

        # model_settings is required in livekit-agents v1.4 — must pass through
        try:
            async for frame in super().tts_node(_cleaned_gen(), model_settings):
                yield frame
        except Exception as e:
            logger.error(f"[TTS-NODE] super().tts_node failed: {e}")
...
async def entrypoint(ctx: JobContext):
    ...
    if stt_provider == "deepgram":
        ...
        agent_stt = sarvam.STT(
            ...
            model="saaras:v3",
            mode="transcribe",          # transcribe keeps original language text
            flush_signal=True,
